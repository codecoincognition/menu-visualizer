...
async function generateFoodImage(foodName: string, description: string): Promise<string> {
  console.log(`Generating AI image for: ${foodName} - ${description}`);
  
  try {
    // Create a detailed prompt for food image generation
    const prompt = `Generate a high-quality, professional food photography image of ${foodName}. ${description}. The image should show an appetizing, well-lit, restaurant-quality presentation on a clean background. Professional food styling with good lighting and composition.`;
    ...
    if (response.candidates && response.candidates[0]?.content?.parts) {
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData && part.inlineData.data) {
          console.log(`Successfully generated AI image for ${foodName}`);
          return `data:${part.inlineData.mimeType};base64,${part.inlineData.data}`;
        }
      }
    }
    
    console.log(`No image generated by AI for ${foodName}, using fallback`);
    // Fallback to curated food images if AI generation fails
    return getFallbackFoodImage(foodName);
    
  } catch (error) {
    console.error("Error generating AI food image:", error);
    console.log(`Using fallback image for ${foodName}`);
    return getFallbackFoodImage(foodName);
  }
}

function getFallbackFoodImage(foodName: string): string {
